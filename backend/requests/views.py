...
def complete_request(request, id):
    ...
    try:
        # Allow both sender and receiver to mark as completed
        ...
        if swap_request.sender != request.user and swap_request.receiver != request.user:
            return Response({
                'error': 'Forbidden',
                'message': 'You are not part of this swap request'
            }, status=status.HTTP_403_FORBIDDEN)
        
        # Mark completion based on who is completing
        if swap_request.sender == request.user:
            if swap_request.sender_completed:
                return Response({
                    'error': 'Bad Request',
                    'message': 'You have already marked this as completed'
                }, status=status.HTTP_400_BAD_REQUEST)
            swap_request.sender_completed = True
            other_user = swap_request.receiver
            other_user_name = f"{other_user.first_name} {other_user.last_name}"
        else:
            if swap_request.receiver_completed:
                return Response({
                    'error': 'Bad Request',
                    'message': 'You have already marked this as completed'
                }, status=status.HTTP_400_BAD_REQUEST)
            swap_request.receiver_completed = True
            other_user = swap_request.sender
            other_user_name = f"{other_user.first_name} {other_user.last_name}"
        
        # If both have completed, mark the swap as completed
        if swap_request.sender_completed and swap_request.receiver_completed:
            ...
            Notification.objects.create(
                user=swap_request.sender,
                message=f"Your skill swap with {swap_request.receiver.first_name} has been completed by both parties!"
            )
            Notification.objects.create(
                user=swap_request.receiver,
                message=f"Your skill swap with {swap_request.sender.first_name} has been completed by both parties!"
            )
        else:
            swap_request.save()
            # Notify the other user that one person has marked it complete
            Notification.objects.create(
                user=other_user,
                message=f"{request.user.first_name} has marked the skill swap as completed. Mark it complete on your end too!"
            )
