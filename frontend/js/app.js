                ...
                {requests.map((request) => {
                    ...
                    return (
                        <div key={request.id} className="bg-[#111827] border border-gray-700 rounded-2xl shadow-md p-6 hover:border-purple-500/30 hover:-translate-y-1 transition-all duration-200">
                            ...
                            {/* Progress Bar for Accepted Swaps */}
                            {isAccepted && (
                                <div className="mb-6 bg-slate-900/30 p-5 rounded-xl border border-slate-700/50">
                                    <div className="flex justify-between items-center mb-3">
                                        <h4 className="text-sm font-semibold text-slate-300">Completion Progress</h4>
                                        <span className="text-xs text-slate-400">
                                            {request.sender_completed && request.receiver_completed ? '100%' : 
                                             request.sender_completed || request.receiver_completed ? '50%' : '0%'}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex justify-between text-xs">
                                        <div className="flex items-center gap-2">
                                            <span className={`w-2 h-2 rounded-full ${request.sender_completed ? 'bg-green-400' : 'bg-slate-600'}`}></span>
                                            <span className={request.sender_completed ? 'text-green-400' : 'text-slate-500'}>
                                                {request.sender.first_name} {request.sender_completed ? '✓' : ''}
                                            </span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <span className={request.receiver_completed ? 'text-green-400' : 'text-slate-500'}>
                                                {request.receiver.first_name} {request.receiver_completed ? '✓' : ''}
                                            </span>
                                            <span className={`w-2 h-2 rounded-full ${request.receiver_completed ? 'bg-green-400' : 'bg-slate-600'}`}></span>
                                        </div>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="flex flex-wrap gap-3">
                                ...
                                {isAccepted && (
                                    <>
                                        ...
                                        {activeTab === 'received' && !request.receiver_completed && (
                                        ...
                                        {activeTab === 'received' && request.receiver_completed && !request.sender_completed && (
                                            <span className="bg-green-500/20 text-green-400 border border-green-500/30 px-6 py-3 rounded-xl font-medium text-sm">
                                                ✓ You marked complete - Waiting for partner
                                            </span>
                                        )}
                                        {activeTab === 'sent' && !request.sender_completed && (
                                            <button onClick={() => { updateRequestStatus(request.id, 'complete'); setReviewModal({ open: true, request }); }} className="bg-gradient-to-r from-purple-500 to-indigo-500 text-white px-6 py-3 rounded-xl hover:brightness-110 hover:scale-105 transition-all duration-200 font-medium text-sm shadow-lg shadow-purple-500/20">
                                                ✓ Mark as Completed
                                            </button>
                                        )}
                                        {activeTab === 'sent' && request.sender_completed && !request.receiver_completed && (
                                            <span className="bg-green-500/20 text-green-400 border border-green-500/30 px-6 py-3 rounded-xl font-medium text-sm">
                                                ✓ You marked complete - Waiting for partner
                                            </span>
                                        )}
